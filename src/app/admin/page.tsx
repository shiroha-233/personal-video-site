              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input 
                    ...
                    placeholder="输入封面图片URL或点击提取封面"
                  />
                  <button 
                    type="button"
                    onClick={extractCover}
                    disabled={isExtractingCover || !formData.videoUrl.trim()}
                    className="bg-purple-600 text-white px-4 py-2 rounded-lg hover:bg-purple-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors whitespace-nowrap"
                  >
                    {isExtractingCover ? '提取中...' : '提取封面'}
                  </button>
                </div>
                {formData.coverImage && (
                  <div className="mt-2">
